...
class LevelThree extends Scene {
    ...
    public LevelThree(Stage primaryStage, double WIDTH, double HEIGHT, int prefCrossHair, int prefGround) {
        ...
        primaryStage.getScene().addEventFilter(KeyEvent.KEY_PRESSED, event -> {
            if (event.getCode() == KeyCode.ESCAPE && lostLevel) {
                ...
                duckFallClip.stop();
                gunShotClip.stop();
                primaryStage.setScene(new LevelFour(primaryStage, WIDTH, HEIGHT, prefCrossHair, prefGround));
            }
        ...
        redDuck = incubator.hatchHorizontal(layout, SCALE, WIDTH, HEIGHT, -1, 1, "red", primaryStage);
